...
library ManagedPoolTokenStorageLib {
    ...
    uint256 private constant _END_NORM_WEIGHT_OFFSET =
        _START_NORM_WEIGHT_OFFSET + _NORM_WEIGHT_WIDTH;
    uint256 private constant _DECIMAL_DIFF_OFFSET =
        _END_NORM_WEIGHT_OFFSET + _NORM_WEIGHT_WIDTH;
     ...
     */
    function getTokenScalingFactor(
        bytes32 tokenState
    ) internal pure returns (uint256) {
        uint256 decimalsDifference = tokenState.decodeUint(
     ...
     */
    function getTokenStartAndEndWeights(
        bytes32 tokenState
    )
        internal
        pure
    ...
    {
        normalizedStartWeight = tokenState.decodeUint(
            _START_NORM_WEIGHT_OFFSET,
            _NORM_WEIGHT_WIDTH
        );
        normalizedEndWeight = tokenState.decodeUint(
            _END_NORM_WEIGHT_OFFSET,
            _NORM_WEIGHT_WIDTH
        );
    }
     ...
     */
    function setTokenScalingFactor(
        bytes32 tokenState,
        IERC20 token
    ...
    ) internal view returns (bytes32 tokenState) {
        tokenState = setTokenScalingFactor(bytes32(0), token);
        tokenState = setTokenWeight(
            tokenState,
            normalizedWeight,
            normalizedWeight
        );
